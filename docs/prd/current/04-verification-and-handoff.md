...
도구 예산 검증 조건 수정: 2026-09-25 (사용자 확인; 다른 보정안 상태 불변)
...
| PRD 02/03 | 한 Run에서 동일 도구를 8회 실행한 뒤 9번째 시도 | `completed` | 9번째 원천 실행 0, 정제된 `budget_exhausted`, 거짓 음성 없음 |
| PRD 02/04 | 한 Run에서 서로 다른 두 도구를 각각 8회 호출 | 정책 상태 유지 | 총 16회 순차 실행, 예산 오류 0 |
...
- 동시 콜백 시도에서 Run별 각 도구의 원자적 8회 예산
- 시간 초과 후 지연 도구 콜백
