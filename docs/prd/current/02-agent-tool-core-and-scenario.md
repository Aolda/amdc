...
진단 에이전트와 도구 코어의 권한·시나리오·인계 생산자 계약을 정의한다. 진단 기준 시각은 저장된 Run 시작 시각으로 고정하며 환경은 모델에 노출하지 않는다. 최신 시각 매핑 보정은 팀 승인 대기다.

상태: 현재
최종 검토: 2026-09-11 (리뷰 보정안, 팀 승인 대기)
도구 예산 결정 반영: 2026-09-25 (사용자 확인; 다른 보정안 상태 불변)
...
`ToolExecutionContext.environment`는 AMDC 도구 코어의 내부 실행 문맥이며
모델 가시 입력이 아니다. 진단/리포트 에이전트 입력 스키마, system/user/tool
message, 에이전트 표시 도구 설명자와 도구 인수에는 환경 값이나 환경 변수 키를
포함하지 않는다. 에이전트는 어떤 환경이 선택됐는지 알거나 변경하지 않고,
도구 코어가 이미 고정된 Run 환경에 맞는 원천만 해석한다.

`diagnosticReferenceTime`은 PRD 01의 `queued -> running` 트랜잭션에서 고정한
`runs.started_at`을 읽어 사용한다. 전이 커밋 전에는 도구를 호출하지 않는다.
지표의 현재 구간과 로그 구간은 이 시각에 끝나고, 지표 기준 구간은
...
1. 등록된 도구 확인
2. 호출 시도를 Run의 해당 도구 예산에 원자적으로 반영
3. 에이전트 입력 스키마 검증
...
등록된 도구의 잘못되거나 차단된 호출 시도도 해당 도구 예산을 소비한다.
알 수 없는 도구 이름은 예산에 반영하지 않고 원천 실행 없이 차단한다. 시간 초과는 `Promise.race`로
...
- 한 Run에서 등록 도구별 호출 시도: 최대 8회
- Run 전체 도구 호출의 별도 합산 한도: 없음
...
도구 호출 예산은 도구 코어에 들어오는 세 `DiagnosticTool` 각각의 호출 시도를 센다.
...
실행한다. 해당 도구의 남은 호출 횟수, Run 기한 또는 증거 예산을 넘는 호출은 실행하지 않고
...
- 각 agent prompt version ID는 애플리케이션 릴리스에서 정확히 하나의 변경 불가
  prompt artifact를 식별한다. prompt 내용을 바꾸면 새 version ID를 발급하고 기존 ID를
  재사용하지 않는다. 시작 시 두 agent의 version-to-artifact 매핑을 검증하며 prompt
  전문은 Run 메타데이터나 로그에 저장하지 않는다.
- Diagnostic prompt artifact와 version은 queued Run 삽입 전에 함께 검증·선택하고
  `diagnostic_prompt_version`으로 고정한다. `DiagnosticAgentPort`에는 그 Run에 고정된
  artifact와 version을 전달하며 현재 구성을 다시 읽어 대체하지 않는다.
- 제공자/모델 ID, `diagnostic_prompt_version`, 조건부
  `report_prompt_version`, 도구 집합 버전을 PRD 01 Run 메타데이터에 저장한다.
- 한 Run 안에서 동일 에이전트의 모든 모델 호출은 저장된 동일 프롬프트 버전만
  사용한다. adapter가 현재 구성으로 버전을 대체하거나 Run 도중 버전을 변경하면
  제공자를 호출하기 전에 닫힌 실패 처리한다.
