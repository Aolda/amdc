...
도구 예산 설명 수정: 2026-09-25 (사용자 확인; 다른 보정안 상태 불변)
...
| `budget_exhausted` | 해당 도구명의 Run별 8회 호출 예산 소진 | 진단 도구 호출 예산이 소진됐다. |
...
AMDC가 상태별 요약, 탐지된 문제, 후속 조치도 결정적으로 조립한다. 아래 고정 문자열은 `schemas/monitoring-report.schema.json` 1.1.0의 `const`와 정확히 같아야 한다. 문체 교정으로 상수를 번역하거나 바꾸지 않으며, 변경이 필요하면 스키마 버전과 소비자 계약을 함께 검토한다.

| 상태 | 고정 요약 | `detected_problem` | 고정 `recommended_next_action` |
|---|---|---|---|
| `problem_detected` | 저장된 Evidence에서 문제 신호가 확인되었다. | 호출 순서상 첫 양성 `Evidence.summary`의 정확한 텍스트 | positive Evidence와 관련 dependency 상태를 수동 확인하고 담당자에게 에스컬레이션한다. |
...
4. `problem_detected`이면 PRD 01/06의 `report_prompt_version` binding을 커밋한 뒤 도구 없는 리포트 에이전트를 최대 한 번 호출하고 출력을
...
7. 검사를 통과한 정제된 도구 인수·증거·도구 오류를 동일 Run에 영속화하고 커밋 확인
8. 저장된 산출물로 ReportAgentInput을 조립하고 정확한 스키마/동일 Run/비밀정보 검사.
   `problem_detected`인 경우에만 PRD 01/06의 Report prompt binding 커밋 후 리포트 제공자 호출
9. 리포트 제공자 출력 스키마/크기/비밀정보 검사 후 7필드 조립과
   리포트 스키마/의미/최종 비밀정보 검사. 제공자 미호출 상태는 원인을 `null`로 조립
10. 리포트 삽입과 `running -> completed` 전이만 PRD 01 완료 트랜잭션으로 원자적 저장
11. 영속화된 리포트 전용 Discord 투영을 최종 비밀정보 검사한 뒤 한 번 전달

7의 저장이 실패하면 인계 조립과 리포트 제공자 호출은 0회다. 7 뒤 제공자 호출 중
중단되더라도 커밋된 안전한 증거/오류는 유지하고 PRD 01 시작 복구를 적용한다.
증거 저장과 리포트 완료 트랜잭션을 합쳐 제공자 호출 뒤로 미루지 않는다.
